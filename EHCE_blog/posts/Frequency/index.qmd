...
slopes_ece_poi_area_adj <- event_count_ece %>%
  ...
  do({
    ...
    tryCatch({
      ...
      percent_change = (exp(coef_model) - 1) * 100
      p_values = summary(model)$coefficients["year_numerical", "Pr(>|z|)"]
      aic_value = AIC(model)
      bic_value = BIC(model)

      data.frame(coef = coef_model, percent_change, p_value = p_values, 
                 AIC = aic_value, BIC = bic_value)
    }, error = function(e) {
      # Return NA in case of an error
      data.frame(coef = NA, percent_change = NA, p_value = NA, 
                 AIC = NA, BIC = NA)
    })
  })
...
state_ehe_dat = event_count_ehe %>% 
  select(year_numerical, STATE_NAME, event_count) 
...
slopes_state_ehe <- poi_state_ehe %>%
  ...
  ungroup() %>%
  select(STATE_NAME, slope, percentage_change,p_value,
         residual_deviance, df_residual, 
